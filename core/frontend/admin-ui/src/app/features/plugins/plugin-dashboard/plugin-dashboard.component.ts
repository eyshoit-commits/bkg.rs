...
import { GooseConfigComponent } from '../goose/goose-config.component';
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, RouterLink, RepoagentConfigComponent, GooseConfigComponent],
...
export class PluginDashboardComponent implements OnInit, OnDestroy {
  ...
  private readonly featureCatalog: Record<string, PluginFeatureDescription[]> = {
    ...
    goose: [
      { title: 'Load-Tests', description: 'HTTP-Lasttests mit konfigurierbaren Goose-Szenarien durchführen.' },
      { title: 'Metriken', description: 'Latenzen, Durchsatz und Fehlerraten in Echtzeit verfolgen.' },
      { title: 'Historie', description: 'Vergangene Läufe auditieren und vergleichen.' },
    ],
    apikeys: [
  ...
  async saveConfig(): Promise<void> {
    ...
    try {
      ...
      if (this.plugin?.name === 'repoagent') {
        ...
        this.gooseSettings = undefined;
      } else if (this.plugin?.name === 'goose') {
        this.gooseSettings = this.extractGooseSettings(config);
  ...
  private updateConfigState(config: PluginConfig): void {
    ...
    if (this.plugin?.name === 'repoagent') {
      ...
      this.gooseSettings = this.extractGooseSettings(cloned);
      this.repoAgentSettings = undefined;
    } else {
      this.repoAgentSettings = undefined;
      this.gooseSettings = undefined;
  ...
  onGooseSettingsChange(settings: GooseSettings): void {
    if (!this.plugin || this.plugin.name !== 'goose' || !this.currentConfig) {
      return;
    }
    this.gooseSettings = settings;
    const updated: PluginConfig = {
      ...this.currentConfig,
      settings: { ...settings },
    };
  ...
  private extractGooseSettings(config: PluginConfig | undefined): GooseSettings | undefined {
    if (!config?.settings) {
      return undefined;
    }
    const settings = config.settings as Partial<GooseSettings>;
    return {
      defaultTarget: settings.defaultTarget ?? '',
      users: settings.users ?? 25,
      ...
      schedule: Array.isArray(settings.schedule)
        ? settings.schedule.map((entry) => ({
            ...
            query: { ...(entry.query ?? {}) },
            thinkTimeMs: entry.thinkTimeMs ?? 0,
          }))
        : [],
    };
  }
